...
function ratioSolution(items, capacity) {
  ...
  for (let key in items) {
    const objVal = items[key].value;
    sortedItems.push(objVal);
  }
  sortedItems = sortedItems.sort((a,b) => b > a);
  sortedItems.forEach((num) => {
    
  });
    // const Itemvalue = items[key].value;
    // if (Itemvalue > items[key].size) {
    //   const underCapacity = size + Itemvalue;
    //   if (underCapacity < capacity) {
    //     size += items[key].size;
    //     value += Itemvalue;
